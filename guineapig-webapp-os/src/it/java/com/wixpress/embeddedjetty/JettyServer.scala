package com.wixpress.embeddedjetty
...
class JettyServer {
  ...
  private val logger = LoggerFactory.getLogger(classOf[JettyServer])
  ...
  def start(): Unit = {
    ...
    validateWebXmlFile()
    validateWarRoot()
//    writeSessionPublicKeysIntoConfig()
...
//  private def writeSessionPublicKeysIntoConfig() = {
//    SessionTestConfiguration.writeToFile()
//  }
//
//  private def writeManifest(): Unit = {
//    writtenManifestFile = Some {
//      (manifest getOrElse FakeManifest.fromModulePom).writeToTarget()
//    }
//  }
...
object JettyServer {
  ...
  def apply(port: Int = DefaultPort,
            ...
            contextRoot: String = DefaultContextRoot): JettyServer = {
    try{
...
//  val server = apply() //additionalResources = Seq("/applicationContext.xml"))
//  server.start()
}
