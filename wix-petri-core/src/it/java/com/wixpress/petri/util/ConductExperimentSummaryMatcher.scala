...
object ConductExperimentSummaryMatcher {
   def hasSummary(experimentId: Int, experimentValue: String, count: Long): Matcher[ConductExperimentSummary] = {
     new TypeSafeMatcher[ConductExperimentSummary]()  {
      protected def matchesSafely(item: ConductExperimentSummary): Boolean ={
         item.experimentId == experimentId && (item.experimentValue == experimentValue) && item.fiveMinuteCount == count && item.totalCount == count
      }
